...
canton_point = local_raw.groupby(['abbreviation_canton','lat', 'long']).size().reset_index().rename(columns={0:'count'})
...
merged = canton_point.merge(merged, left_on='abbreviation_canton', right_on='Canton')
...
merged2 = merged.iloc[1].to_json()
print(merged2)

geosource = GeoJSONDataSource(geojson=merged2)
...
mappers = {} 
mappers['Density'] = linear_cmap(field_name=demo_raw.data['Density'],palette=bp.Inferno256, low=demo_raw.Density.min(), high=demo_raw.Density.max())
mappers['BedsPerCapita'] = linear_cmap(field_name=demo_raw.data['BedsPerCapita'],palette=bp.Inferno256, low=demo_raw.BedsPerCapita.min(), high=demo_raw.BedsPerCapita.max())
...
color_bar = ColorBar(color_mapper=mappers['Density'], width=16, location=(0,0), title="P_rate")
